...
Création : **22 septembre 2026**. Reprise : **23 septembre 2026**.
Statut : **note de conception et de réévaluation du besoin**.
...
**Bénéfice central confirmé le 23 septembre : pouvoir déléguer des tâches
variées à un agent sans devoir surveiller ni administrer constamment ses
actions, tout en réduisant les risques.** L'adéquation de Moraine à ce besoin
reste à établir.

**Question de conception initiale : comment Moraine peut-elle contrôler réellement les
...
| --- | --- |
| Déléguer avec moins de risques, de surveillance et d'administration | Bénéfice recherché explicitement confirmé le 23 septembre |
...
| Moraine comme passerelle ou proxy qui applique les permissions | Direction confirmée initialement; capacité à résoudre le besoin remise en question dans la reprise du 23 septembre |
| Agent qui formule les requêtes; Moraine qui contrôle, fait exécuter et restitue | Fonctionnement de référence clarifié avec l'utilisateur le 23 septembre : contrôle à l'aller et au retour; intégration concrète non implémentée ni qualifiée |
| Valeur propre d'un connecteur unique | Hypothèse soulevée par l'utilisateur le 23 septembre, même si les permissions sont très larges; valeur et différenciation non établies |
...
| Consultation permise avec exclusions; écriture et envoi bloqués avec autorisations ciblées | Principe confirmé par l'utilisateur le 23 septembre; périmètres, priorités, effets et contrat détaillé restent à préciser |
...
### Réévaluation du 23 septembre : utilité et friction

L'utilisateur souligne qu'une utilisation irritante ou compliquée pourrait
conduire rapidement les personnes à tout autoriser. C'est un risque produit
exprimé, sans mesure d'usage à ce stade. La friction des permissions doit donc
faire partie de l'évaluation de leur efficacité réelle.

Deux hypothèses de valeur sont désormais à examiner séparément :

- **Faciliter l'accès aux services** par un connecteur unique. Ce bénéfice
  pourrait subsister avec des permissions larges. L'utilisateur envisage
  qu'il puisse suffire à justifier le produit; ce n'est ni une validation de
  marché ni une décision d'abandonner le contrôle des permissions.
- **Réduire les risques de délégation sans surveillance constante**. Des
  contrôles techniquement applicables ne suffisent pas à établir ce bénéfice
  si les personnes doivent sans cesse les régler, les approuver ou les désactiver.

L'exemple « supprimer les publicités » servait à illustrer la diversité des
besoins, pas à commander une fonctionnalité de nettoyage. Les scénarios doivent
éprouver un mécanisme commun plutôt que devenir chacun un développement métier.

Une limite centrale a été clarifiée : **une passerelle ne connaît pas
automatiquement le mandat utilisateur**. L'appel d'outil et la justification
de l'agent ne constituent pas une preuve indépendante de ce mandat. Le recevoir
directement de l'utilisateur ou d'une application authentifiée demanderait une
intégration supplémentaire, non choisie. Le réviseur évoqué en section 5 ne
peut donc pas être supposé disposer de cette information. Contrôler une
permission et apprécier la conformité à l'intention sont des capacités distinctes.

Le pilote apporte des preuves techniques bornées, pas une preuve de l'utilité
quotidienne de ces deux propositions. La suite doit réexaminer le bénéfice et
la place de Moraine avant de prolonger la conception de la passerelle. Aucune
nouvelle architecture, offre ou implémentation n'est décidée par cette reprise.
...
| Rôle dans le fonctionnement envisagé | Responsabilité |
| --- | --- |
| Agent | Conduire l'exploration, formuler les requêtes adressées à Moraine, lire les résultats restitués, recouper et produire le bilan |
| Moraine | Contrôler la demande, décider de son exécution, l'exécuter ou la faire exécuter par un connecteur contrôlé, puis contrôler et restituer le résultat à l'agent |
| Outil ou connecteur | Traduire et exécuter l'opération autorisée auprès du fournisseur, puis rendre le résultat à Moraine avant toute transmission à l'agent |
...
**Fonctionnement de référence clarifié le 23 septembre : l'agent formule la
requête; Moraine contrôle son exécution et le retour du résultat.** Pour les
accès protégés, Moraine est le passage obligatoire dans les deux sens :

1. **Demande de l'agent.** L'agent choisit l'opération et ses paramètres, puis
   adresse sa demande à Moraine. Il conserve l'initiative de la recherche.
2. **Contrôle à l'aller.** Moraine vérifie l'identité, le compte, les droits,
   l'opération réelle et ses paramètres. Elle autorise, bloque ou demande
   l'approbation prévue par la politique. Un refus ou une attente ne déclenche
   pas l'opération fournisseur.
3. **Exécution contrôlée.** Moraine effectue l'appel fournisseur ou le confie
   à un connecteur soumis à ses contrôles. Elle ne remet pas simplement un
   avis favorable à l'agent pour qu'il exécute ensuite librement l'opération.
4. **Contrôle au retour.** Moraine reçoit le résultat avant l'agent, applique
   les exclusions de données et la chaîne de validation, d'inspection et de
   publication prévue. Elle restitue le contenu autorisé ou un état explicite
   de retenue, d'erreur ou de résultat partiel. Le contenu brut ne doit pas
   atteindre l'agent avant ces contrôles.

« Moraine fait la requête » décrit cette responsabilité d'exécution; le code
qui émet l'appel réseau peut être celui d'un connecteur réutilisé. Son contrat
doit rendre ses effets contrôlables et empêcher une restitution directe à
l'agent. Les accès contournant Moraine ne sont pas couverts par ce fonctionnement.
Le transport, les adaptateurs et les garanties d'isolation restent à définir
et à qualifier; cette description n'annonce pas une intégration déjà livrée.
...
La proposition hybride ci-dessous explore ces choix sans les arrêter.
...
### Principe hybride du 23 septembre : exclusions en lecture, autorisations en écriture

L'utilisateur envisage d'abord une politique permissive avec des exclusions
explicites, puis propose deux comportements par défaut selon l'opération :

| Famille | Comportement proposé par défaut | Personnalisation |
| --- | --- | --- |
| Consultation et recherche | Permettre dans les comptes rendus accessibles à l'agent | Ajouter des exclusions de lecture sur des ressources ou périmètres identifiés |
| Écriture, modification, suppression et envoi | Bloquer | Ajouter des autorisations ciblées sur des opérations et périmètres identifiés |

Cette proposition concerne des comptes et des agents expressément reliés;
elle n'accorde aucun accès à un agent inconnu ou à toutes les connexions de
la personne. L'utilisateur confirme le principe de ces défauts; les modalités
et le contrat détaillé ne sont ni acceptés ni implémentés à ce stade.

Une interface pourrait présenter « Consultation : autorisée, sauf… » et
« Modifications et envois : bloqués, sauf… ». Les permissions seraient définies
pour les capacités des outils; il ne serait pas nécessaire de coder chaque
besoin exprimé à l'agent ou de connaître le mandat de chaque conversation.

**Le refus par défaut et l'interdiction explicite doivent rester distincts.**
Proposition de priorité à discuter : une autorisation ciblée peut lever le
refus par défaut de sa catégorie, tandis qu'une interdiction explicite applicable
continue de bloquer. Par exemple, permettre les modifications dans un dossier
ne lèverait pas une interdiction explicite de suppression définitive. L'agent
ne pourrait ni créer ces exceptions ni modifier les règles par ses seuls appels.

Une éventuelle approbation ponctuelle pourrait permettre une action exacte
sans créer une autorisation durable. Ce parcours reste à décider; un blocage
ne signifie pas automatiquement une demande d'approbation, et une interdiction
explicite ne deviendrait pas contournable en sollicitant un accord.

Le compromis de confidentialité doit être visible : les données non exclues
du périmètre de consultation seraient accessibles, même sans rapport avec la
tâche. Une permission d'écriture ne devrait pas lever implicitement une exclusion
de lecture. Les exclusions doivent couvrir aussi les extraits de recherche et
les autres chemins de restitution concernés.

La classification doit suivre les effets de l'opération. Un outil nommé
« consulter » peut aussi modifier un état; une opération mixte doit faire
contrôler chacun de ses effets. Le traitement des opérations inconnues ou dont
les effets ne sont pas établis reste à définir : leur nom seul ne suffit pas
à les faire bénéficier de la consultation autorisée.

Cette piste vise à réduire les sollicitations pendant l'exploration tout en
gardant les effets externes sous contrôle. Son utilité et sa friction ne sont
pas validées; elle ne résout pas la vérification de l'intention utilisateur.
...
Le fonctionnement de référence de la section 3 impose le contrôle avant
exécution et avant restitution. **Le point d'interception technique permettant
de l'assurer n'est pas encore décidé.**
...
### Inspection des réponses avec des requêtes choisies par l'agent

**Fonctionnement envisagé au 23 septembre :** conserver le filtrage des contenus
entrants alors que l'agent peut conduire librement ses recherches, selon les
responsabilités explicitées en section 3. L'intégration n'est pas qualifiée.
La couche d'inspection est prévue, mais non implémentée dans le
[contrat d'ingestion proposé](../plans/email-ingestion-contract.md).

Le parcours fonctionnel envisagé est :

```mermaid
flowchart LR
    A[Agent] -->|Requête| P[Moraine : permissions]
    P --> F[Connecteur et fournisseur]
    F -->|Résultat| I[Moraine : inspection et restitution]
    I -->|Contenu publiable et autorisé| A
```

Dans ce fonctionnement, « requête directe » désigne une requête formulée
librement par l'agent et adressée à Moraine. L'appel fournisseur et le retour
du résultat restent sous le contrôle de Moraine. Un appel qui la contourne
et remet directement la réponse à l'agent échappe à cette protection. Le point
d'interception concret reste à choisir; le diagramme n'impose ni MCP ni un
proxy HTTP universel.

Le contrat d'ingestion initial part d'une sélection humaine fermée avant
publication. La recherche progressive demanderait d'appliquer la chaîne
pertinente à chaque contenu externe destiné à l'agent, au fur et à mesure :
validation et décodage, inspection, décision de publication, puis contrôle
du droit de restitution. Elle ne demande pas de pré-sélectionner manuellement
chaque message, mais exige une évolution explicite de ce contrat.

**Consultation autorisée ne signifie pas inspection désactivée.** Les règles
de permission déterminent les données accessibles; la politique de publication
détermine le traitement d'une alerte, d'une analyse incomplète ou d'une panne.
L'agent doit recevoir un état explicite lorsque du contenu est retenu, sans
présenter cela comme une recherche sans résultat. Cette politique reste à décider
pour la passerelle; celle du contrat initial n'est pas implicitement adoptée.

La couverture doit inclure les extraits de recherche, corps, métadonnées et
tout autre contenu fournisseur rendu à l'agent. Pièces jointes, téléchargements,
liens et caches exigent un chemin contrôlé s'ils sont exposés. Un flux ne doit
pas livrer le contenu brut avant le contrôle prévu. Un connecteur qui agit
lui-même sur le contenu avant ce retour nécessite aussi un contrôle de ses
effets internes; filtrer sa réponse ne peut annuler une action déjà exécutée.

Le filtrage ne garantit pas l'absence d'injection. Assainissement technique,
détection et décision de publication restent distincts; retirer du HTML ne
supprime pas nécessairement une instruction malveillante en texte ordinaire.
Les permissions restent applicables même si une attaque échappe au détecteur.
Voir la [note d'inspection](email-content-inspection-sources.md) et les
[défenses OWASP](https://cheatsheetseries.owasp.org/cheatsheets/LLM_Prompt_Injection_Prevention_Cheat_Sheet.html),
consultées le 23 septembre. Aucun détecteur ni parcours de filtrage dynamique
n'est qualifié par cette discussion.
...
4. **Exécuter ce qui est autorisé.** Moraine réalise l'appel fournisseur ou
   le fait réaliser par le connecteur contrôlé. Pour un envoi approuvé, conserver le lien
...
5. **Contrôler la restitution.** Appliquer les exclusions de données et
   l'inspection/publication avant transmission. Restituer les données permises
   avec leur source et les limites de l'opération, ou l'état explicite qui
   explique leur indisponibilité.
...
**Sujet en cours : réévaluer l'adéquation de Moraine au bénéfice confirmé en
section 1.** Examiner la valeur d'un connecteur unique et celle d'un contrôle
des permissions utilisable sans surveillance constante, sans présumer que
l'une ou l'autre justifie un produit. La position de passerelle est une
hypothèse à réexaminer, pas une contrainte à préserver à tout prix.

Le contrat minimal d'une requête inspectable et les réglages de la section 5
restent des travaux ouverts. Leur conception ne doit pas présumer l'accès au
mandat utilisateur, ni transformer chaque exemple de tâche en fonction métier.
Le principe hybride est confirmé : consultation autorisée avec exclusions,
écriture et envoi bloqués avec autorisations ciblées. Ses priorités et le
parcours après blocage restent à discuter. La section 6 examine le maintien
de l'inspection des réponses dans ce parcours, sans qualification technique.
La section 3 définit le fonctionnement de référence : l'agent formule les
requêtes, Moraine contrôle leur exécution à l'aller et leurs résultats au retour.
...
21–23 septembre 2026. Les constats sur le pilote proviennent des documents
