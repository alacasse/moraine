...
Mise à jour documentaire : 20 septembre 2026, après le laboratoire local.
Les résultats ci-dessous sont ceux des validations enregistrées; cette mise à
jour de documentation ne constitue pas une nouvelle exécution des campagnes.
...
Linux, puis 102 avec le laboratoire. Les neuf scénarios sont exécutés aussi par
...
- **Ingestion/inspection** : le [contrat B](plans/email-ingestion-contract.md) a
  ...
  d'ingestion. Les ressources du pilote sont encore fournies par `create_grant`
  sur le canal humain. Le lab emploie des ressources fictives par ce même chemin.
...
- **Client et fournisseur externes** : le serveur MCP n'est pas installé dans
  la configuration Codex. OAuth/Gmail et une livraison email réelle ne sont pas
  implémentés ou validés. Le mandat actuel privilégie les services métier locaux.
