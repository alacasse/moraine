...
Date : **21 septembre 2026**. **Exécution autorisée par l'utilisateur après
revue indépendante. Code implémenté dans le workspace; 122 tests réussis et
parcours Codex natif validé avec canal humain simulé. Aucun commit/push demandé.**

Le [rapport du lot](../../pilot-results/codex-email/20260921-agent-access/REPORT.md)
conserve les sources testées, les appels Codex, la récupération observée et
le refus de lecture après révocation.

Précision de l'utilisateur pendant l'exécution : **simuler le canal humain,
car l'humain ne peut pas recevoir la demande dans cet essai**. L'opérateur
de test effectue donc la revue et la décision par la CLI/socket; les appels
Codex restent natifs. Cette simulation ne qualifie pas un consentement humain
indépendant ni une réception de notification.

La [revue indépendante](../../pilot-results/agent-access/20260921-plan-review/REPORT.md)
conserve une capture du plan avant autorisation. Les descriptions du socle
ci-dessous désignent l'état antérieur au lot, au commit indiqué.
...
Même clé avec un contenu différent : conflit explicite. Une autre clé visant
une demande en attente ou un accès actif reçoit `access_already_exists` sans
création ni consommation de cette clé. Le client consulte `get_access` pour
retrouver l'existant. Une nouvelle tentative explicite avec cette clé refusée
peut créer une demande après refus/expiration de la précédente.
Un second clic d'approbation ne lance pas une seconde
...
| Expiration pendant la récupération | `approved` + `failed`, motif `activation_expired`; une lecture fournisseur possible, aucun grant ni ressource publiée, aucune relance par consultation ou redémarrage |
...
de Codex avec le canal humain simulé par l'opérateur de test, conformément
à la précision de l'utilisateur pendant l'exécution. Le seul client
...
La mention des outils MCP dans [AGENTS.md](../../AGENTS.md) a été actualisée
avec l'autorisation explicite de l'utilisateur nommant ce fichier.
...
expérience. L'implémentation et l'essai local de ce lot ont été autorisés;
sa publication Git et les intégrations réelles restent hors du mandat.
