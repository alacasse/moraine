...
**Question de reprise du 23 septembre :** conserver le filtrage des contenus
entrants alors que l'agent peut conduire librement ses recherches.
Cette compatibilité est une proposition d'architecture, pas une intégration
qualifiée. La couche d'inspection est prévue, mais non implémentée dans le
...
« Requête directe » peut signifier que l'agent choisit lui-même les paramètres
d'un appel qui transite par Moraine. Cela reste compatible avec une inspection
de sa réponse. Un appel qui contourne Moraine et remet directement la réponse
à l'agent échappe en revanche à ce contrôle. Le point d'interception concret
reste à choisir; le diagramme n'impose ni MCP ni un proxy HTTP universel.
