...
- [Comparaison avec Arcade et Workato](plans/agent-access-competitive-comparison.md) :
  plan proposé pour observer deux agents, des accès distincts, la reprise et
  la révocation; essais externes non autorisés.
...
Pour lancer la comparaison revue dans une autre session :
[prompt Arcade / Workato](prompts/run-agent-access-comparison.md).
Il reprend la préparation puis les essais selon les accès et autorisations disponibles.
...
- [Marché entreprise de l'accès des agents](research/enterprise-agent-access-market.md) :
  offres existantes, recouvrements et hypothèses de positionnement.
- [Prérequis de comparaison Arcade/Workato](research/agent-access-comparison-prerequisites.md) :
  accès aux éditions, identité utilisateur et connexion Confluence à vérifier.
...
- [Application des permissions dans la passerelle](research/permission-enforcement-gateway.md) :
  reprise du 22 septembre 2026; inspection des requêtes, recherches successives,
  règles générales, approbation des envois et journal d'activité. Orientations
  retenues, propositions et questions ouvertes; aucun mandat d'implémentation.
