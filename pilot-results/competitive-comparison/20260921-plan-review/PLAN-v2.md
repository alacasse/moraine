...
Les versions examinées, constats et verdicts sont conservés dans le
[rapport de revue](../../pilot-results/competitive-comparison/20260921-plan-review/REPORT.md).
...
deux agents logiques **A** et **B**. U est un utilisateur ordinaire, non
administrateur, avec un compte distinct de O. A et B peuvent employer le même logiciel
...
L'édition source doit permettre ces restrictions : Confluence Free ne permet
pas de personnaliser les permissions et restrictions. Vérifier l'édition et
la durée de l'essai source, ses conditions de fin et de facturation, avant de
préparer D4. Si ce prérequis manque, D4 reste « bloqué par prérequis source »;
les autres contrôles peuvent continuer si leurs droits initiaux sont vérifiés.
[Permissions et restrictions Confluence](https://support.atlassian.com/confluence-cloud/docs/what-are-confluence-cloud-permissions-and-restrictions/).
...
| --- | --- | --- |
| S0 — Contrôle de la source | O crée le corpus; U consulte directement la source. | U lit D1–D3 et ne lit pas D4. Conserver état initial, IDs, versions et droits source. Sans contrôle initial d'une ressource, ses refus ultérieurs ne peuvent pas être attribués correctement; isoler cette limite dans les résultats. |
...
| S2 — Accès distincts | A et B lisent D1, tentent D2 puis D3/D4, par ID et via les surfaces de découverte exposées. Avec les seuls identifiants de B, viser aussi le point d'entrée ou sélecteur public associé à A lorsque cette substitution existe. | Réponses conformes à la matrice; B ne lit jamais D2, y compris en changeant de point d'entrée. Toute divulgation hors périmètre est un écart, même si le résumé final l'omet. Si le produit n'a pas d'identité A/B distincte, noter la limite et ne pas présenter deux utilisateurs comme un succès. |
...
| S5 — Révocation de A | O retire uniquement l'accès de A, en préservant le compte et les droits source de U ainsi que tous les accès de B. Préserver la connexion commune si elle existe; retirer une connexion propre à A est permis lorsqu'elle matérialise sa délégation. | Depuis le contexte ouvert puis un contexte vide, A ne lit plus D1/D2; B continue de lire D1 et reste refusé sur D2. Consigner nombre de connexions, consentements supplémentaires, objet révoqué et couche responsable. Un retrait global peut être noté comme alternative, jamais comme une révocation isolée réussie. |
...
Pour le contrôle croisé S2, ne copier aucun secret de A et ne solliciter aucune
nouvelle autorisation humaine. Rester sur les points d'entrée du tenant
d'essai autorisé, sans balayage ni recherche de vulnérabilité externe. Conserver
l'élément serveur rattachant la décision à la délégation de B. Si aucune
substitution n'existe, documenter pourquoi et quelle preuve serveur équivalente
lie les droits à l'agent. Sans contrôle applicable ou preuve équivalente,
la séparation A/B reste « non établie », même si les parcours ordinaires passent.
